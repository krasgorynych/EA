...
def func_all(vect, k):
    if k == 0:
        return (vect[0] - 2)**2 +  (vect[1] - 1)**2 + 2
    if k == 1:
        return 9*vect[0] -  (vect[1] - 1)**2 
'''  
'''   
def func_all(vect, k):
    if k == 0:
...
number_iteration = 50
GDnsga = [0]*number_iteration
Snsga = [0]*number_iteration
GDspea = [0]*number_iteration
Sspea = [0]*number_iteration

for iter in range(number_iteration):
    
    objgansga = NSGAII(a, 0.1, 50, 100, 0.1, 2, 2)
    
    objgansga.randomizer()
    for i in range(objgansga.number_population):
    ...
    massiv_two = [objgansga.matrix_phenotype[i] for i in range(objgansga.number_individ) if objgansga.matrix_rank[i] == 1]
    
    GDnsga[iter] = generational_distance(func_all, objgansga.matrix_phenotype, massiv_two, objgansga.number_criteria)
    Snsga[iter] = spasing(func_all, objgansga.matrix_phenotype, objgansga.number_criteria)
        
    x = objgansga.fitness_func(func_all, 0).copy()
    y = objgansga.fitness_func(func_all, 1).copy()
    fig, ax = plt.subplots()
    ax.scatter(x, y)
    ax.grid()
    ax.set_xlabel('f1(x)')
    ...
    objgaspea = SPEA2(a, 0.1, 50, 100, 0.1, 2, 2, 25)
    
    objgaspea.randomizer()
    help_matrix_gen = objgaspea.matrix_genotype.copy()
    ...
    for i in range(objgaspea.number_population):
        objgaspea.conver_genotype_phenotype()
    ...
    massiv_two = [objgaspea.matrix_phenotype_archive[i] for i in range(objgaspea.number_individ_archive) if help_vector[i] == 0]
    
    GDspea[iter] = generational_distance(func_all, objgaspea.matrix_phenotype_archive, massiv_two, objgaspea.number_criteria)
    Sspea[iter] = spasing(func_all, objgaspea.matrix_phenotype_archive, objgaspea.number_criteria)
    
    # x = objgaspea.fitness_func(func_all, 0).copy()
    # y = objgaspea.fitness_func(func_all, 1).copy()
    # fig, ax = plt.subplots()
    # ax.scatter(x, y)
    # ax.grid()
    # ax.set_xlabel('f1(x)')
...
print(sum(GDnsga) / len(GDnsga))
print(sum(Snsga) / len(Snsga))
print(sum(GDspea) / len(GDspea))
print(sum(Sspea) / len(Sspea))